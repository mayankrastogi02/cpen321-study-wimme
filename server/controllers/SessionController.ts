  ...
  async hostSession(req: Request, res: Response) {
    try {
      ...
      if (new Date(dateRange.startDate) >= new Date(dateRange.endDate)) {
        return res
          .status(400)
          .json({ message: "Start date must be earlier than the end date" });
      }
      ...
      for (const inviteeId of savedSession.invitees) {
        await sendPushNotification(
          ...
          `${host.userName} invited you to session '${savedSession.name}'`
        );
      }
  ...
  async joinSession(req: Request, res: Response) {
    try {
      ...
      if (!session.isPublic && !session.invitees.includes(userId)) {
        return res
          .status(400)
          .json({ message: "User is not invited to this private session" });
      }
      ...
      await sendPushNotification(
        session.hostId,
        "User Joined Session",
        `${user.userName} joined session '${session.name}'`
      );

      res
        .status(200)
        .json({ message: "User joined session successfully", session });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Internal server error" });
    }
  ...
  async leaveSession(req: Request, res: Response) {
    try {
      ...
      await sendPushNotification(
        session.hostId,
        "User Left Session",
        `${user.userName} left session '${session.name}'`
      );

      res
        .status(200)
        .json({ message: "User left session successfully", session });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Internal server error" });
    }
